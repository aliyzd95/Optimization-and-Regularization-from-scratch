...
def gradient_descent_with_l2(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    ...
    for e in range(epochs):
        for i in range(0, train_X.shape[0], s):
            ...
            derivatives = backward_prop_with_l2(X, Y, store, m, L, af, af_choices, parameters)
        ...
        if e % 1 == 0:
            ...
            cost = crossentropy_with_l2(AA, train_Y, parameters, L)
...
def gradient_descent_with_l1(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
    ...
    for e in range(epochs):
        for i in range(0, train_X.shape[0], s):
            ...
            Y = train_Y[i:s + i, :]
            A, store = forward_prop(X, parameters, L, af, af_choices)
            derivatives = backward_prop_with_l1(X, Y, store, m, L, af, af_choices, parameters)
            for l in range(1, L + 1):
                parameters[f'W{str(l)}'] -= l_rate * derivatives[f'dW{str(l)}']
                parameters[f'b{str(l)}'] -= l_rate * derivatives[f'db{str(l)}']
        if e % 1 == 0:
            AA, _ = forward_prop(train_X, parameters, L, af, af_choices)
            cost = crossentropy_with_l1(AA, train_Y, parameters, L)
            acc = pred(train_X, train_Y, parameters, L, af, af_choices)
            costs.append(cost)
            accs.append(acc)
            print(f'epoch: {e} - cost= {cost} - Train Acc= {acc}')
...
def adagrad_with_l2(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
...
def adagrad_with_l1(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
...
def rmsprop_with_l2(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
...
def rmsprop_with_l1(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
...
def adam_with_l2(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
...
def adam_with_l1(train_X, train_Y, epochs, l_rate, layers, L, af, af_choices, batch_size):
    s = batch_size
    m = s
    parameters = initialize_parameters(layers, L)
    costs = []
